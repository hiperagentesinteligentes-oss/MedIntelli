...
-- 1. USUARIOS DO SISTEMA (ja devem existir, mas garantir que existam)
...
-- Nota: usuarios sao criados via Supabase Auth, aqui apenas garantimos perfis

-- 2. PACIENTES PARA TESTE (20 pacientes)
...
('Joao Pedro Oliveira', '222.333.444-55', '(11) 98765-4322', 'joao.pedro@email.com', '1990-07-22', 'CASSI', true, NOW()),
('Ana Paula Costa', '333.444.555-66', '(11) 98765-4323', 'ana.paula@email.com', '1978-11-08', 'CABESP', true, NOW()),
('Carlos Eduardo Lima', '444.555.666-77', '(11) 98765-4324', 'carlos.eduardo@email.com', '1982-05-30', 'PARTICULAR', true, NOW()),
('Fernanda Souza', '555.666.777-88', '(11) 98765-4325', 'fernanda.souza@email.com', '1995-09-12', 'UNIMED', true, NOW()),
('Roberto Santos', '666.777.888-99', '(11) 98765-4326', 'roberto.santos@email.com', '1970-02-18', 'UNIMED', true, NOW()),
('Patricia Alves', '777.888.999-00', '(11) 98765-4327', 'patricia.alves@email.com', '1988-06-25', 'CASSI', true, NOW()),
('Ricardo Ferreira', '888.999.000-11', '(11) 98765-4328', 'ricardo.ferreira@email.com', '1975-12-03', 'CABESP', true, NOW()),
('Juliana Martins', '999.000.111-22', '(11) 98765-4329', 'juliana.martins@email.com', '1992-04-17', 'PARTICULAR', true, NOW()),
('Bruno Henrique', '000.111.222-33', '(11) 98765-4330', 'bruno.henrique@email.com', '1987-08-09', 'UNIMED', true, NOW()),
('Camila Rodrigues', '111.222.333-00', '(11) 98765-4331', 'camila.rodrigues@email.com', '1993-01-20', 'UNIMED', true, NOW()),
('Diego Araujo', '222.333.444-11', '(11) 98765-4332', 'diego.araujo@email.com', '1980-10-15', 'CASSI', true, NOW()),
('Elaine Cristina', '333.444.555-22', '(11) 98765-4333', 'elaine.cristina@email.com', '1989-03-28', 'CABESP', true, NOW()),
('Fabricio Gomes', '444.555.666-33', '(11) 98765-4334', 'fabricio.gomes@email.com', '1976-07-11', 'PARTICULAR', true, NOW()),
('Gabriela Mendes', '555.666.777-44', '(11) 98765-4335', 'gabriela.mendes@email.com', '1994-11-05', 'UNIMED', true, NOW()),
('Henrique Batista', '666.777.888-55', '(11) 98765-4336', 'henrique.batista@email.com', '1983-02-14', 'UNIMED', true, NOW()),
('Isabela Carvalho', '777.888.999-66', '(11) 98765-4337', 'isabela.carvalho@email.com', '1991-06-19', 'CASSI', true, NOW()),
('Jorge Luiz', '888.999.000-77', '(11) 98765-4338', 'jorge.luiz@email.com', '1972-09-23', 'CABESP', true, NOW()),
('Kelly Fernandes', '999.000.111-88', '(11) 98765-4339', 'kelly.fernandes@email.com', '1986-12-30', 'PARTICULAR', true, NOW()),
('Leonardo Dias', '000.111.222-99', '(11) 98765-4340', 'leonardo.dias@email.com', '1979-05-07', 'UNIMED', true, NOW())
ON CONFLICT (cpf) DO NOTHING;

-- 3. FERIADOS NACIONAIS E MUNICIPAIS DE 2025
...
-- 4. AGENDAMENTOS DE TESTE (5+ agendamentos em diferentes datas)
...
-- 5. LISTA DE ESPERA (5 pacientes novos + 3 cadastrados)
-- Primeiro, inserir pacientes novos para lista de espera
INSERT INTO pacientes (nome, cpf, telefone, email, data_nascimento, convenio, ativo, created_at) VALUES
('Marcos Antonio', '100.200.300-40', '(11) 99999-1111', 'marcos.antonio@email.com', '1984-04-10', 'UNIMED', true, NOW()),
('Vanessa Lima', '200.300.400-50', '(11) 99999-2222', 'vanessa.lima@email.com', '1992-08-25', 'CASSI', true, NOW()),
('Paulo Roberto', '300.400.500-60', '(11) 99999-3333', 'paulo.roberto@email.com', '1977-12-18', 'CABESP', true, NOW()),
('Luciana Pires', '400.500.600-70', '(11) 99999-4444', 'luciana.pires@email.com', '1989-03-07', 'PARTICULAR', true, NOW()),
('Anderson Silva', '500.600.700-80', '(11) 99999-5555', 'anderson.silva@email.com', '1995-11-22', 'UNIMED', true, NOW())
ON CONFLICT (cpf) DO NOTHING;
...
-- 6. MENSAGENS APP (criar algumas mensagens de teste para pacientes)
WITH pacientes_teste AS (
  SELECT id FROM pacientes ORDER BY created_at DESC LIMIT 5
)
...
SELECT
  id,
  CASE 
    WHEN row_number() OVER () % 2 = 0 THEN 'lembrete'
    ELSE 'notificacao'
  END,
  CASE 
    WHEN row_number() OVER () % 2 = 0 THEN 'Voce tem uma consulta agendada para amanha as 10h'
    ELSE 'Seus exames ja estao disponiveis para download'
  END,
  'enviado',
  NOW()
FROM pacientes_teste;

-- 7. CRIAR TABELA DE VALIDACOES SE NAO EXISTIR
...
-- 8. INSERIR CHECKLIST DE VALIDACAO
...
-- CONCLUIDO: Dados de teste inseridos com sucesso
