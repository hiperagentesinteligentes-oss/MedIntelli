...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    // Criar novo AbortController para esta requisicao com timeout de 20s
    ...
    try {
      ...
      if (error.name === 'AbortError') {
        console.log('Requisicao abortada por timeout (20s)');
        const timeoutMessage: ChatMessage = {
          id: `timeout-${Date.now()}`,
          role: 'assistant',
          content: 'O sistema esta temporariamente lento. Pode repetir sua pergunta?',
          timestamp: new Date().toISOString(),
        };
