...
export default function AgendaPage() {
  ...
  const loadAgendamentos = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/agendamentos?start=${start.toISOString()}&end=${end.toISOString()}`,
        {
          headers: {
      ...
      (data.data || []).forEach((agendamento: Agendamento) => {
        const dateKey = format(new Date(agendamento.data_agendamento), 'yyyy-MM-dd');
    ...
    try {
      ...
      const response = await fetch(`/api/agendamentos`, {
        ...
        body: JSON.stringify({
          ...
          data_agendamento: dataHoraCompleta.toISOString(),
          duracao_minutos: editFormData.duracao_minutos,
    ...
    try {
      ...
      const response = await fetch(`/api/agendamentos`, {
        ...
        body: JSON.stringify({
          ...
          data_agendamento: format(dataAgendamento, 'yyyy-MM-dd'),
          hora_agendamento: quickScheduleTime,
          tipo_consulta: quickFormData.tipo_consulta,
          duracao_minutos: quickFormData.duracao_minutos,
            ...
            <>
              ...
              {weekDays.map((day) => {
                const dayAgendamentos = agendamentos.filter(a => 
                  isSameDay(new Date(a.data_agendamento), day) &&
                  a.hora_agendamento === time
                );
  ...
  const renderDayView = () => {
    const dayAgendamentos = agendamentos.filter(a => 
      isSameDay(new Date(a.data_agendamento), currentDate)
    );
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-200 max-h-[600px] overflow-y-auto">
          {TIME_SLOTS.map((time) => {
            const timeAgendamentos = dayAgendamentos.filter(a => a.hora_agendamento === time);
  ...
  const agendamentosDodia = selectedDate
    ? agendamentos.filter(a => isSameDay(new Date(a.data_agendamento), selectedDate))
    : [];
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          <span className="font-semibold">
                            {agendamento.hora_agendamento || format(new Date(agendamento.data_agendamento), 'HH:mm')}
                          </span>
