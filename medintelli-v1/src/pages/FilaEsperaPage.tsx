...
export default function FilaEsperaPage() {
  ...
  const searchPatients = async (term: string) => {
    ...
    setSearching(true);
    try {
      // Buscar diretamente no Supabase REST API
      const supabaseUrl = 'https://ufxdewolfdpgrxdkvnbr.supabase.co';
      const response = await fetch(
        `${supabaseUrl}/rest/v1/pacientes?or=(nome.ilike.%25${encodeURIComponent(term)}%25,telefone.ilike.%25${encodeURIComponent(term)}%25)&limit=10`,
        {
          headers: {
  ...
  const loadFila = async () => {
    ...
    try {
      const response = await fetch(
        `${FUNCTION_URL}/fila-espera?status=aguardando`,
        {
          headers: {
            'Authorization': `Bearer ${session.access_token}`,
            'Content-Type': 'application/json',
  ...
  const handleRemover = async (id: string) => {
    ...
    try {
      const response = await fetch(`${FUNCTION_URL}/fila-espera?id=${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
          'Content-Type': 'application/json',
        ...
        {showForm && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="mb-6">
                ...
                <div className="mt-2 border border-gray-200 rounded-lg max-h-48 overflow-y-auto">
                  {searchResults.map((patient) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-600">{patient.telefone} {patient.convenio && `• ${patient.convenio}`}</div>
                    </button>
                  ))}
                </div>
              )}
              ...
              {selectedPatient && (
                <div className="mt-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-green-900">{selectedPatient.nome}</div>
                    <div className="text-xs text-green-700">{selectedPatient.telefone}</div>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
              ...
              {!selectedPatient && searchTerm && !searching && searchResults.length === 0 && (
                <p className="mt-2 text-sm text-gray-500">
                  Nenhum paciente encontrado. Preencha o formulário abaixo para cadastro rápido.
                </p>
              )}
            </div>
