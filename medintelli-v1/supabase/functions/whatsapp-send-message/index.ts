// Edge Function 2.1 - Agendamentos (Patch v5 - Correções App Paciente)
// Autor: Sistema MedIntelli
// Data: 2025-11-11
// Finalidade: CRUD de agendamentos com validação RLS, auditoria e filtros corrigidos
// Correções: 1) Cadastro rápido quando paciente_id ausente 2) Verificação de conflito corrigida 
//            3) Insert com status 'pendente' 4) GET com filtros por status (pendente,confirmado)
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        // Validação JWT do Supabase (aceita ANON_KEY para sistema customizado)
        ...
        if (req.method === 'GET') {
            ...
            let query = `${supabaseUrl}/rest/v1/agendamentos?select=*,paciente:pacientes(id,nome,telefone,convenio)&status=in.(pendente,confirmado)&order=data_agendamento.asc`;
        ...
        // POST: Criar novo agendamento
            ...
            const conflictResponse = await fetch(
                `${supabaseUrl}/rest/v1/agendamentos?data_agendamento=eq.${data_agendamento}&status=in.(pendente,confirmado)&select=id&limit=1`,
                {
                    headers: {
                        'Authorization': `Bearer ${serviceRoleKey}`,
                        'apikey': serviceRoleKey
        ...
        // PUT: Atualizar agendamento existente
            ...
            if (data_agendamento) {
                ...
                const conflictResponse = await fetch(
                    `${supabaseUrl}/rest/v1/agendamentos?data_agendamento=eq.${data_agendamento}&id=neq.${id}&status=in.(pendente,confirmado)&select=id&limit=1`,
                    {
                        headers: {
                            'Authorization': `Bearer ${serviceRoleKey}`,
                            'apikey': serviceRoleKey
